...
export default function ServicePageClient({ service, related }: Props) {
  ...
  return (
    <>
      ...
      {/* ── Hero & Overview ─────────────────────────────────────── */}
      <section className="relative min-h-screen lg:h-screen flex flex-col justify-center overflow-hidden bg-navy pt-24 pb-12">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 pt-8 flex-grow flex flex-col justify-center">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-white/45 text-sm mb-6">
          ...
          <div className="grid lg:grid-cols-5 gap-10 lg:gap-14 items-start">
            {/* Left side: text */}
            <ScrollReveal from="left" className="lg:col-span-3 text-white">
              <div>
                <span className="inline-block text-brand text-xs font-bold tracking-widest uppercase mb-3">{ui.overlineTag}</span>
                <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight mb-4 leading-[1.08]">{title}</h1>
                <p className="text-white/60 text-lg md:text-xl max-w-xl mb-6">{tagline}</p>
                
                <div className="h-px bg-white/10 my-6 max-w-xl" />
                
                <span className="inline-block text-brand text-sm font-semibold tracking-widest uppercase mb-3">{ui.overlineOverview}</span>
                <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight mb-4 leading-snug">{ui.overviewTitle}</h2>
                <p className="text-white/80 text-[15px] sm:text-base leading-relaxed mb-8 max-w-2xl">{overview}</p>
                
                <Link href={contactPath(lang)} className="inline-flex items-center gap-2 bg-brand hover:bg-brand-dark text-white font-semibold px-7 py-3.5 rounded-xl transition-all duration-200 shadow-lg shadow-brand/10 hover:shadow-brand/20 hover:scale-[1.02]">
                  {ui.overviewCta}
                </Link>
              </div>
            </ScrollReveal>

            {/* Right side: Highlights box styled as Frosted Glass (Glassmorphism) */}
            <ScrollReveal from="right" className="lg:col-span-2 w-full">
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-7 border border-white/15 shadow-2xl text-white">
                <h3 className="text-sm font-bold text-brand tracking-widest uppercase mb-5">{ui.highlightsTitle}</h3>
                <ul className="flex flex-col gap-4">
                  {highlights.map((h, i) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span className="text-white/90 text-sm font-medium leading-relaxed">{h}</span>
                    </li>
                  ))}
                </ul>
                <div className="mt-8 pt-6 border-t border-white/15">
                  <p className="text-xs text-white/40 uppercase tracking-wider font-semibold mb-3">{ui.contactLabel}</p>
                  <a href="mailto:agency@neosmaritime.com" className="flex items-center gap-3 group">
                    ...
                    <span className="text-white font-semibold text-sm group-hover:text-brand transition-colors">agency@neosmaritime.com</span>
                  </a>
                </div>
              </div>
            </ScrollReveal>
      ...
      <main>
        ...
        {/* ── How We Work ──────────────────────────────────────── */}
        <section className="py-12 sm:py-16 bg-navy overflow-hidden">
          <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
            <ScrollReveal from="bottom">
              <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        {/* ── CTA ──────────────────────────────────────────────── */}
        <section className="py-12 sm:py-16 bg-slate-50 border-t border-gray-100 overflow-hidden">
          <ScrollReveal from="bottom">
            <div className="max-w-3xl mx-auto px-5 text-center">
              ...
              <p className="text-slate-500 text-lg mb-6 leading-relaxed">{ui.ctaSub}</p>
